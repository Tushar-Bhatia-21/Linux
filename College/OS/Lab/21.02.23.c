...
int main() 
{
    ...
    scanf("%d", &n);
    int pid[n], arrival_time[n], burst_time[n], remaining_time[n], completed[n],priority[n];
    
    for (int i = 0; i < n; i++) 
    {
        printf("Enter the arrival time and burst time of process %d: ", i + 1);
        scanf("%d %d", &arrival_time[i], &burst_time[i]);
        printf("Enter the priority of each process %d: ",i+1);
        scanf("%d",&priority[i]);
        pid[i] = i + 1;
...
void calculateTime(int process[], int n, int arrival[], int burst[],int priority[])
{
 ...
 int current_time = 0;
    ...
    printf("\nProcess\t Arrival Time\t Priority\t Burst Time\t Exit Time\t Turn Around Time\t Waiting Time\n");
    for (int i = 0; i < n; i++)
        printf("%d\t\t %d\t\t %d\t\t %d\t\t %d\t\t %d\t\t %d\n", process[i], arrival[i], priority[i],burst[i], exit[i], turn_around[i], waiting_time[i]);
...
int main()
{
    ...
    int process[n], arrival[n], burst[n],priority[n];
...
//SORTING ARRIVAL AND BURST TIME AND PRiORITY
    int temp;
    for(int i=0;i<n;i++)
    {   
        for(int j=i+1;j<n;j++)
        {
            ...
            if(priority[i]>priority[j] || priority[i]==priority[j] )
            {
                temp=priority[i];
    ...
    calculateTime(process, n, arrival, burst,priority);
    return 0;
}
